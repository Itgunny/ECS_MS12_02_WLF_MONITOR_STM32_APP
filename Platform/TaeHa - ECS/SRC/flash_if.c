...
/**
  ...
  *         1: error occurred
  */
uint32_t FLASH_If_SMK_Erase(uint32_t StartSector)
{
...
uint32_t FLASH_If_WriteByte(__IO uint32_t* FlashAddress, uint8_t* Data ,uint32_t DataLength)
{
  uint32_t i = 0;
  ...
  for (i = 0; (i < DataLength) && (*FlashAddress <= (USER_FLASH_END_ADDRESS-4)); i++)
  {
    /* Device voltage range supposed to be [2.7V to 3.6V], the operation will
       be done by word */ 
    if (FLASH_ProgramByte(*FlashAddress, *(uint8_t*)(Data+i)) == FLASH_COMPLETE)
    {
